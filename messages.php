<?php
	...
	if (isset($_POST['mailcommand'])) {
		...
		if ($_POST['mailcommand'] == "delete") {
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET deleted = 'Y', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id IN $inSQL ";
			
		} else if ($_POST['mailcommand'] == "markread") {
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET status = 'R', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id IN $inSQL ";
			
		} else if ($_POST['mailcommand'] == "markunread") {
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET status = 'N', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id IN $inSQL ";
		}
	...
	<?php
		if (! isset($_GET['mode']) || $_GET['mode'] == "I") {
			$qry = "SELECT A.id, A.replied, A.status, A.subject, A.message, A.from_member_id, A.to_member_id, " .
					 ...
					 "LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members C " .
					 "ON C.member_id = A.to_member_id " .
					 "LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members B " .
