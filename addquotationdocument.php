<?php
	...
	$qry = "SELECT A.id FROM {$_SESSION['ESCCO_DB_PREFIX']}documents A " .
		   "WHERE A.sessionid = '" . session_id() . "' " .
		   	"AND A.id NOT IN (SELECT documentid FROM {$_SESSION['ESCCO_DB_PREFIX']}quotationdocs WHERE documentid = A.id) " .
		   "ORDER BY A.id";
	...
	while (($member = mysql_fetch_assoc($result))) {
		$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}quotationdocs " .
