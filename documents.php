<?php
	...
	function delete() {
		if (isset($_POST['pk1'])) {
			$id = $_POST['pk1'];
			$qry = "DELETE FROM {$_SESSION['ESCCO_DB_PREFIX']}documents WHERE id = $id";
		...
		<?php
			if (isset($_GET['id'])) {
				$qry = "SELECT A.*, DATE_FORMAT(A.createddate, '%d/%m/%Y') AS createddate, " .
						...
						"FROM {$_SESSION['ESCCO_DB_PREFIX']}documents A " .
						"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members B " .
						"ON B.member_id = A.createdby " .
						"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}" . $_GET['table'] . " C " .
				...
				$qry = "SELECT A.*, DATE_FORMAT(A.createddate, '%d/%m/%Y') AS createddate, " .
						...
						"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members B " .
						"ON B.member_id = A.createdby " .
						"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}trainingcode C " .
						"ON C.id = A.headerid " .
						$where . " " .
						"ORDER BY A.id";
			}
