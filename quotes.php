...
?>
			...
			function redirectEdit(id) {
				callAjax(
						"finddata.php", 
						{ 
							sql: "SELECT caseid FROM <?php echo $_SESSION['ESCCO_DB_PREFIX'];?>quotes WHERE id = " + id
						},
						function(data) {
							if (data.length == 1) {
...
<?php			
	...
	$crud->table = "{$_SESSION['ESCCO_DB_PREFIX']}quotes";
	...
	if (isset($_GET['id'])) {
		$crud->sql = 
				...
				"FROM {$_SESSION['ESCCO_DB_PREFIX']}quotes A " .
				"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}cases B " .
				"ON B.id = A.caseid " .
				"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}courts BV " .
				"ON BV.id = B.courtid " .
				"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members D " .
				"ON D.member_id = A.contactid " .
				"LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}courts E " .
				"ON E.id = B.clientcourtid " .
				"LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}offices F " .
				"ON F.id = A.officeid " .
