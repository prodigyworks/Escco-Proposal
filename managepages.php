<?php
	...
	function sequenceUp() {
		...
		$qry = "SELECT sequence " .
				"FROM {$_SESSION['ESCCO_DB_PREFIX']}pagenavigation A " .
		...
		if ($maxsequenceid != -1) {
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}pagenavigation " .
	...
	function saveContent() {
		...
		$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}pages SET content = '$contentvalue', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE pageid = $pageid";
		$result = mysql_query($qry);
	...
	class PageEdit extends Crud {
		...
		public function postInsertEvent() {
			...
			$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}pageroles (pageid, roleid, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUE($pageid, 'PUBLIC', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
			$result = mysql_query($qry);
			...
			if ($_POST['menuitem'] == "Y") {
				$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}pagenavigation (pageid, childpageid, sequence, pagetype, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUE($parentpageid, $pageid, $maxSequence, 'M', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
				
			} else {
				$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}pagenavigation (pageid, childpageid, sequence, pagetype, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUE($parentpageid, $pageid, $maxSequence, 'L', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
			}
		...
		public function preCommandEvent() {
			if (isset($_POST['rolecmd'])) {
				...
				$pageid = $_POST['pageid'];
				$qry = "DELETE FROM {$_SESSION['ESCCO_DB_PREFIX']}pageroles WHERE pageid = $pageid";
				$result = mysql_query($qry);
				...
				for ($i = 0; $i < $counter; $i++) {
					...
					$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}pageroles (pageid, roleid, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUES ($pageid, '$roleid', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
					$result = mysql_query($qry);
				};
			}
		}
			...
			<div id="roleDialog" class="modal">
				<form id="rolesForm" name="rolesForm" method="post">
					...
					<select class="listpicker" name="roles[]" multiple="true" id="roles" >
						<?php createComboOptions("roleid", "roleid", "{$_SESSION['ESCCO_DB_PREFIX']}roles", "", false); ?>
					</select>
				</form>
			</div>
<?php
...
?>
			...
			function editContent(pageid) {
				...
				callAjax(
						"finddata.php", 
						{ 
							sql: "SELECT content FROM <?php echo $_SESSION['ESCCO_DB_PREFIX'];?>pages WHERE pageid = " + pageid
						},
						function(data) {
...
<?php
	...
	$crud = new PageEdit();
	$crud->title = "Pages";
	$crud->table = "{$_SESSION['ESCCO_DB_PREFIX']}pages";
	$crud->dialogwidth = 600;
	
	if (isset($_GET['id'])) {
		$crud->sql = 
				"SELECT A.*, B.pagetype " .
				"FROM {$_SESSION['ESCCO_DB_PREFIX']}pages A " .
				"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}pagenavigation B " .
