...
function start_db() {
	...
	if (! isset($_SESSION['ESCCO_PRODIGYWORKS.INI'])) {
		$_SESSION['ESCCO_PRODIGYWORKS.INI'] = parse_ini_file("prodigyworks.ini");
		$_SESSION['ESCCO_DB_PREFIX'] = $_SESSION['ESCCO_PRODIGYWORKS.INI']['DB_PREFIX']; 
		$_SESSION['ESCCO_CACHING'] = $_SESSION['ESCCO_PRODIGYWORKS.INI']['CACHING']; 
	}
	
	if (! defined('ESCCO_DB_HOST')) {
		$iniFile = $_SESSION['ESCCO_PRODIGYWORKS.INI'];
		
		define('ESCCO_DB_HOST', $iniFile['DB_HOST']);
	    define('ESCCO_DB_USER', $iniFile['DB_USER']);
	    define('ESCCO_DB_PASSWORD', $iniFile['DB_PASSWORD']);
	    define('ESCCO_DB_DATABASE', $iniFile['DB_DATABASE']);
	    define('ESCCO_DEV_ENV', $iniFile['DEV_ENV']);
	    
		//Connect to mysql server
		$link = mysql_connect(ESCCO_DB_HOST, ESCCO_DB_USER, ESCCO_DB_PASSWORD);
		...
		//Select database
		$db = mysql_select_db(ESCCO_DB_DATABASE);
		
		if(!$db) {
			logError("Unable to select database:" . ESCCO_DB_DATABASE);
		}
		
		mysql_query("BEGIN");
	
		if (! isset($_SESSION['ESCCO_SITE_CONFIG'])) {
			$qry = "SELECT * FROM {$_SESSION['ESCCO_DB_PREFIX']}siteconfig";
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$_SESSION['ESCCO_SITE_CONFIG'] = $data;
				}
...
function GetUserName($userid = "") {
	if ($userid == "") {
		return $_SESSION['ESCCO_SESS_FIRST_NAME'] . " " . $_SESSION['ESCCO_SESS_LAST_NAME'];
		
	} else {
		$qry = "SELECT * FROM {$_SESSION['ESCCO_DB_PREFIX']}members A " .
...
function GetEmail($userid) {
	$qry = "SELECT email FROM {$_SESSION['ESCCO_DB_PREFIX']}members A " .
			"WHERE A.member_id = $userid ";
	$result = mysql_query($qry);
	$name = "Unknown";
...
function dateStampString($oldnotes, $newnotes, $prefix = "") {
	...
	return 
		mysql_escape_string (
				...
				$prefix . " - " . 
				date("F j, Y, g:i a") . " : " . 
				$_SESSION['ESCCO_SESS_FIRST_NAME'] . " " . 
				$_SESSION['ESCCO_SESS_LAST_NAME'] . "\n" . 
				$newnotes
			);
}
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	$qry = "SELECT B.email, B.firstname, B.member_id FROM {$_SESSION['ESCCO_DB_PREFIX']}userroles A " .
			"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members B " .
...
function sendInternalRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	$qry = "SELECT B.email, B.firstname, B.lastname FROM {$_SESSION['ESCCO_DB_PREFIX']}members B " .
...
function sendTeamMessage($id, $subject, $message, $footer = "") {
	$qry = "SELECT C.member_id, C.email, C.firstname " .
			"FROM {$_SESSION['ESCCO_DB_PREFIX']}members A " .
			"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members C " .
			"ON C.teamid = A.teamid " .
			"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}userroles D " .
...
function isAuthenticated() {
	return ! (!isset($_SESSION['ESCCO_SESS_MEMBER_ID']) || (trim($_SESSION['ESCCO_SESS_MEMBER_ID']) == ''));
}

function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	$qry = "SELECT B.email, B.firstname FROM {$_SESSION['ESCCO_DB_PREFIX']}members B " .
...
function sendMessage($subject, $message, $id, $action = "") {
	$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}messages " .
...
function isUserAccessPermitted($action, $description = "") {
	...
	$pageid = $_SESSION['ESCCO_pageid'];
	$found = 0;
	$actionid = 0;
	$qry = "SELECT A.id " .
			"FROM {$_SESSION['ESCCO_DB_PREFIX']}applicationactions A  " .
	...
	if ($found == 0) {
		$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}applicationactions (pageid, code, description, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUES($pageid, '$action', '$desc', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
		$result = mysql_query($qry);
		...
		$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}applicationactionroles (actionid, roleid, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUES($actionid, 'PUBLIC', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
		$result = mysql_query($qry);
	...
	$found = 0;
	$qry = "SELECT A.* " .
			"FROM {$_SESSION['ESCCO_DB_PREFIX']}applicationactionroles A  " .
			"WHERE A.actionid = $actionid " .
			"AND A.roleid IN (" . ArrayToInClause($_SESSION['ESCCO_ROLES']) . ")";
	$result = mysql_query($qry);
...
function isUserInRole($roleid) {
	for ($i = 0; $i < count($_SESSION['ESCCO_ROLES']); $i++) {
		if ($roleid == $_SESSION['ESCCO_ROLES'][$i]) {
			return true;
		}
	}
...
function getSiteConfigData() {
	return $_SESSION['ESCCO_SITE_CONFIG'];
}
...
function getLoggedOnMemberID() {
	...
	if (! isset($_SESSION['ESCCO_SESS_MEMBER_ID'])) {
		return 0;
	}
	
	return $_SESSION['ESCCO_SESS_MEMBER_ID'];
}
...
function logError($description, $kill = true) {
	...
	if (isset($_SESSION['ESCCO_pageid'])) {
		$pageid = $_SESSION['ESCCO_pageid'];
	...
	$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}errors (pageid, memberid, description, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUES ($pageid, " . getLoggedOnMemberID() . ", '" . mysql_escape_string($description) . "', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
	$result = mysql_query($qry);
...
function cms() {
	$pageid = $_SESSION['ESCCO_pageid'];
	$qry = "SELECT content FROM {$_SESSION['ESCCO_DB_PREFIX']}pages " .
			"WHERE pageid = $pageid";
	$result = mysql_query($qry);
...
function login($login, $password, $redirect = true) {
	...
	$errflag = false;
	unset($_SESSION['ESCCO_LOGIN_ERRMSG_ARR']);
	unset($_SESSION['ESCCO_ERR_USER']);
	unset($_SESSION['ESCCO_MENU_CACHE']);
	...
	$qry = "SELECT DISTINCT A.*, B.name AS customername
		    FROM {$_SESSION['ESCCO_DB_PREFIX']}members A 
		    LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}customer B
	...
	if($result) {
		if(mysql_num_rows($result) == 1) {
			...
			$_SESSION['ESCCO_SESS_MEMBER_ID'] = $member['member_id'];
			$_SESSION['ESCCO_SESS_FIRST_NAME'] = $member['firstname'];
			$_SESSION['ESCCO_SESS_LAST_NAME'] = $member['lastname'];
			$_SESSION['ESCCO_SESS_CUSTOMER_ID'] = $member['customerid'];
			$_SESSION['ESCCO_SESS_CUSTOMER_NAME'] = $member['customername'];
			
			$qry = "SELECT * FROM {$_SESSION['ESCCO_DB_PREFIX']}userroles WHERE memberid = " . $_SESSION['ESCCO_SESS_MEMBER_ID'] . "";
			$result=mysql_query($qry);
			$index = 0;
			$status = null;
			...
			$_SESSION['ESCCO_ROLES'] = $arr;
			
			$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}loginaudit " .
					"(" .
					"memberid, timeon, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid" .
					") " .
					"VALUES " .
					"(" .
					$_SESSION['ESCCO_SESS_MEMBER_ID'] . ", NOW(), NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . "" .
					")";
			$auditresult = mysql_query($qry);
			$auditid = mysql_insert_id();
			
			$_SESSION['ESCCO_SESS_LOGIN_AUDIT'] = $auditid;
			
			if (! $auditresult) {
				logError("$qry - " . mysql_error());
			}
			
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}members SET " .
					"loginauditid = $auditid, metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " " .
					"WHERE member_id = " .
					$_SESSION['ESCCO_SESS_MEMBER_ID'];
			$auditresult = mysql_query($qry);
			...
			$qry = "SELECT lastschedulerun " .
				   "FROM {$_SESSION['ESCCO_DB_PREFIX']}siteconfig A " .
...
//				$errmsg_arr[] = "Login not found / Not active.<br>Please register or contact portal support";
			...
			$_SESSION['ESCCO_LOGIN_ERRMSG_ARR'] = $errmsg_arr;
...
function logout() {
	...
	if (isAuthenticated()) {
		$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}loginaudit SET " .
				"timeoff = NOW(), metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " " .
				"WHERE id = " . $_SESSION['ESCCO_SESS_LOGIN_AUDIT'] . "";
		$result = mysql_query($qry);
	}
	
	session_unset();
	
	$_SESSION['ESCCO_ROLES'][] = 'PUBLIC';
}
...
//			$stti = microtime(true);
	...
	$cachekey = 'ESCCO_FNC_CACHE_' . $functionname . "_" . $encoded;
	
	if (! isset($_SESSION[$cachekey]) || $_SESSION['ESCCO_CACHING'] == "false") {
		ob_start(); //Turn on output buffering 
