<?php
	...
	for ($i = 0; $i < count($pages); $i++) {
		...
		if ($pageid == 0) {
			$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}typistinvoices " .
			...
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}casetypist SET " .
			...
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}cases SET " .
					"datebackfromtypist = NOW(), metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " " .
					"WHERE id = (SELECT caseid FROM {$_SESSION['ESCCO_DB_PREFIX']}casetypist WHERE id = $casetypistid)";
			$result = mysql_query($qry);
			...
			$qry = "SELECT C.j33number, C.casenumber, C.plaintiff, A.pages " .
					"FROM  {$_SESSION['ESCCO_DB_PREFIX']}typistinvoices A " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}casetypist B " .
					"ON B.id = A.casetypistid " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}cases C " .
					"ON C.id = B.caseid " .
			...
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}typistinvoices SET " .
