	...
	class CreditReport extends FPDF
	{
		...
		function __construct($orientation, $metric, $size, $id) {
			...
			$sql = "SELECT A.*, " .
					...
					"FROM {$_SESSION['ESCCO_DB_PREFIX']}invoices A " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}cases B " .
					"ON B.id = A.caseid " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}courts D " .
					"ON D.id = B.courtid " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}province E " .
					"ON E.id = D.provinceid " .
					"LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}caseterms F " .
					"ON F.id = A.termsid " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}members G " .
					"ON G.member_id = A.contactid " .
					"LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}courts I " .
					"ON I.id = B.clientcourtid " .
					"LEFT OUTER JOIN {$_SESSION['ESCCO_DB_PREFIX']}offices J " .
					"ON J.id = A.officeid " .
					"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}offices O " .
					"ON O.id = A.officeid " .
					"WHERE A.caseid = $id " .
					"ORDER BY B.id";
...
//					$description = $this->stripHTML($member['description']);
					...
					$sql = "SELECT C.id, C.description, C.qty, C.vat, C.vatrate, C.total, C.unitprice, " .
							...
							"FROM {$_SESSION['ESCCO_DB_PREFIX']}invoiceitems C " .
							"INNER JOIN {$_SESSION['ESCCO_DB_PREFIX']}invoiceitemtemplates H " .
