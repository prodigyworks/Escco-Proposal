<?php
	...
	function expire() {
		$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}members SET status = 'N', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE member_id = " . $_POST['expiredmemberid'];
	...
	function live() {
		$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}members SET status = 'Y', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE member_id = " . $_POST['expiredmemberid'];
		$result = mysql_query($qry);
	...
	class UserCrud extends Crud {
		...
		public function preCommandEvent() {
			if (isset($_POST['rolecmd'])) {
				...
				$qry = "DELETE FROM {$_SESSION['ESCCO_DB_PREFIX']}userroles WHERE memberid = $memberid";
				$result = mysql_query($qry);
				...
				for ($i = 0; $i < $counter; $i++) {
					...
					$qry = "INSERT INTO {$_SESSION['ESCCO_DB_PREFIX']}userroles (memberid, roleid, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) VALUES ($memberid, '$roleid', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
					$result = mysql_query($qry);
				};
			}
		}
...
<?php
	...
	$crud->title = "Users";
	$crud->table = "{$_SESSION['ESCCO_DB_PREFIX']}members";
