<?php
	...
	if (isset($_POST['mailcommand'])) {
		...
		if ($_POST['mailcommand'] == "delete") {
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET deleted = 'Y', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id = $id ";
			...
			$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET replied = 'Y', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id = $id ";
			
			header("location: messagereply.php?id=$id");
		}
		
	} else {
		$qry = "UPDATE {$_SESSION['ESCCO_DB_PREFIX']}messages SET status = 'R', metamodifieddate = NOW(), metamodifieduserid = " . getLoggedOnMemberID() . " WHERE id = $id ";
